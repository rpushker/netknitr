...
#' @param columns_for_nodes Vector of column names for crude_data, works well for atleast 3 columns
#' @param group Logical value, set TRUE to differentiate nodes by shapes and colors. Default value is FALSE
...
getNodes <-  function(crude_data, columns_for_nodes = NULL, group = FALSE) {
  ...
  if(group == FALSE) {
    ...
    res <- unique(res)
    res <- data.frame(id = 1:length(res), label = res)
  } else if(group == TRUE) {
    if(is.null(columns_for_nodes)) {
      print("----Columns considerd for nodes----->")
      res <- lapply(names(crude_data), FUN = function(x) {
